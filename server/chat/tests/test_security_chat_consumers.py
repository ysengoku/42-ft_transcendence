...
class UserEventsConsumerTests(TransactionTestCase):
    async def get_authenticated_communicator(self, username=None, password=None):
        ...
        assert connected, "Connexion to the WebSocket failed"
        return communicator
    ...
    async def test_uuid_validation(self):
        ...
        with self.assertLogs("server", level="WARNING") as logs:
            ...
            assert any("Invalid UUID format" in log for log in logs.output)
    ...
    async def test_missing_chat_id_field(self):
        ...
        with self.assertLogs("server", level="WARNING") as logs:
            await communicator.receive_nothing()
            assert any("Missing field" in log for log in logs.output)
    ...
    async def test_valid_message_workflow(self):
        ...
        assert response["action"] == "new_message"
        msg_count = await database_sync_to_async(ChatMessage.objects.count)()
        assert msg_count == 1
    ...
    async def test_notification_workflow(self):
        ...
        response = await communicator.receive_json_from()
        assert response["action"] == "new_friend"
