...
export class Game extends HTMLElement {
  ...
  initializeUIElements() {
    console.log('Initializing UI elements (singleton pattern)');

    // Only create scoreElement if it doesn't exist
    if (!this.scoreElement) {
      ...
      console.log('Created scoreElement');
    } else {
      console.log('scoreElement already exists, reusing');
    }

    // Only create timerElement if it doesn't exist
    if (!this.timerElement) {
      ...
      if (timerWrapper && !timerWrapper.querySelector('game-timer')) {
        timerWrapper.appendChild(this.timerElement);
      }
      console.log('Created timerElement');
    } else {
      console.log('timerElement already exists, reusing');
    }

    // Only create buffIconElement if it doesn't exist
    if (!this.buffIconElement) {
      this.buffIconElement = document.createElement('game-buff-icon');
      this.appendChild(this.buffIconElement);
      console.log('Created buffIconElement');
    } else {
      console.log('buffIconElement already exists, reusing');
    }

    // Only create lifePointElement if it doesn't exist
    if (!this.lifePointElement) {
      this.lifePointElement = document.createElement('game-life-point');
      this.appendChild(this.lifePointElement);
      console.log('Created lifePointElement');
    } else {
      console.log('lifePointElement already exists, reusing');
    }
    ...
    if (!this.overlay) {
      ...
      console.log('overlay already exists, reusing');
    }
  }
  ...
  async createRenderer() {
    try {
      ...
      renderer.domElement.addEventListener('webglcontextlost', async (event) => {
      ...
      await this.handleWebGLError(error);
      return null;
    }
  }
  ...
  async handleWebGLError(error) {
    ...
    this.clearAllUIExceptOverlay();
    await this.recreateOverlay();
      ...
      model.traverse((node) => {
        if (node.isMesh) {
          ...
          if (node.material && !node.material.isMaterial) {
            console.warn('Invalid material detected:', node.material);
          }
        }
      });
  ...
  recreateOverlay() {
    console.log('Recreating overlay (singleton pattern)');

    // Remove any existing overlay references
    if (this.overlay) {
      this.overlay.remove();
      this.overlay = null;
    }

    // Clean up any duplicate overlays that might exist
    this.querySelectorAll('game-overlay').forEach((overlay) => {
      console.log('Removing duplicate overlay:', overlay);
      overlay.remove();
    });

    // Force a microtask delay to ensure DOM cleanup is complete
    return new Promise((resolve) => {
      requestAnimationFrame(() => {
        // Create a single new overlay element
        this.overlay = document.createElement('game-overlay');
        this.overlay.gameType = `local-${this.#state.gameType}`;
        this.appendChild(this.overlay);
        console.log('Created single new overlay:', this.overlay);
        resolve();
      });
    });
  }
  ...
  hideGameUI() {
    console.log('HIDING GAME UI - Clean visibility approach');

    // Simply hide elements using display style (no DOM removal)
    if (this.scoreElement) {
      this.scoreElement.style.display = 'none';
      console.log('Hidden scoreElement');
    }

    if (this.timerElement) {
      this.timerElement.style.display = 'none';
      console.log('Hidden timerElement');
    }

    if (this.buffIconElement) {
      this.buffIconElement.style.display = 'none';
      console.log('Hidden buffIconElement');
    }

    if (this.lifePointElement) {
      this.lifePointElement.style.display = 'none';
      console.log('Hidden lifePointElement');
    }

    // Hide the timer wrapper if it exists
    const timerWrapper = document.getElementById('game-timer-wrapper');
    if (timerWrapper) {
      timerWrapper.style.display = 'none';
      console.log('Hidden timerWrapper');
    }
  }

  showGameUI() {
    console.log('SHOWING GAME UI - Restore visibility');

    // Restore visibility of existing elements
    if (this.scoreElement) {
      this.scoreElement.style.display = '';
      console.log('Shown scoreElement');
    }

    if (this.timerElement) {
      this.timerElement.style.display = '';
      console.log('Shown timerElement');
    }

    if (this.buffIconElement) {
      this.buffIconElement.style.display = '';
      console.log('Shown buffIconElement');
    }

    if (this.lifePointElement) {
      this.lifePointElement.style.display = '';
      console.log('Shown lifePointElement');
    }

    // Show the timer wrapper if it exists
    const timerWrapper = document.getElementById('game-timer-wrapper');
    if (timerWrapper) {
      timerWrapper.style.display = '';
      console.log('Shown timerWrapper');
    }
  }

  async handleGameInitError(error) {
    console.error('Game initialization failed:', error);

    // Nuclear cleanup - remove everything except overlay
    this.clearAllUIExceptOverlay();

    // Completely recreate the overlay to prevent any positioning drift
    await this.recreateOverlay();

    this.overlay?.show(OVERLAY_TYPE.ERROR, {
      title: 'Game Failed to Load',
      message: 'The game encountered an error while loading. This might be due to network issues or outdated browser.',
      actions: [
        {
          label: 'Retry',
          onClick: () => this.retryGameInit(),
        },
      ],
    });
  }

  clearAllUIExceptOverlay() {
    console.log('NUCLEAR CLEANUP: Removing ALL children except overlay');

    // Store the overlay temporarily
    const overlayBackup = this.overlay;

    // Nuclear DOM cleanup - remove EVERYTHING (including canvas, UI elements, etc.)
    while (this.firstChild) {
      this.removeChild(this.firstChild);
    }

    // Clear all references to prevent ghost elements
    this.scoreElement = null;
    this.timerElement = null;
    this.buffIconElement = null;
    this.lifePointElement = null;

    // Clean up any canvas elements that might be floating around
    document.querySelectorAll('canvas').forEach((canvas) => {
      if (this.contains(canvas)) {
        canvas.remove();
      }
    });

    // Clean up any three.js renderer elements
    const rendererElements = document.querySelectorAll('canvas[data-engine="three.js r162"]');
    rendererElements.forEach((el) => {
      if (this.contains(el)) {
        el.remove();
      }
    });

    // Clean up timer wrapper if it exists anywhere
    const timerWrapper = document.getElementById('game-timer-wrapper');
    if (timerWrapper) {
    ...
    // Clean up any other game-related elements that might exist
    ['game-scoreboard', 'game-timer', 'game-buff-icon', 'game-life-point'].forEach((tagName) => {
    ...
    // Restore only the overlay
    if (overlayBackup) {
      this.overlay = overlayBackup;
      this.appendChild(this.overlay);
      console.log('Restored only the overlay');
    }

    // Force a style reset on the component
    this.style.height = '';
    this.style.overflow = '';

    console.log('Nuclear cleanup complete - only overlay remains');
  }
  ...
  async retryGameInit() {
    try {
      ...
      // Recreate UI elements fresh
      this.initializeUIElements();

      // Hide the overlay on successful retry
      this.overlay?.hideOverlay();

      // Try to render the game
      await this.render();

      // If successful, show UI
      this.showGameUI();
    } catch (error) {
      // If retry fails, show error with completely clean slate
      console.error('Retry failed:', error);

      // Nuclear cleanup again
      this.clearAllUIExceptOverlay();

      // Recreate overlay for error display
      await this.recreateOverlay();

      this.overlay?.show(OVERLAY_TYPE.ERROR, {
        title: 'Game Still Failed to Load',
        message: 'The retry attempt failed. Please check your internet connection and try again.',
        actions: [
          {
            label: 'Retry Again',
            onClick: () => this.retryGameInit(),
  ...
  async game() {
    try {
      ...
      function isCollidedWithBall(bumper, ballSubtickZ, ballSubtickX) {
        return (
          Ball.sphereUpdate.x - BALL_RADIUS + ballSubtickX * Ball.velocity.x <=
            bumper.cubeUpdate.x + bumper.lenghtHalf &&
          Ball.sphereUpdate.x + BALL_RADIUS + ballSubtickX * Ball.velocity.x >=
            bumper.cubeUpdate.x - bumper.lenghtHalf &&
          Ball.sphereUpdate.z - BALL_RADIUS + ballSubtickZ * Ball.velocity.z <=
            bumper.cubeUpdate.z + bumper.widthHalf &&
      ...
      await this.handleGameInitError(error);
  ...
  async render() {
    ...
    // Check if game initialization failed
